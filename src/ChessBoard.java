    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                    ...
                    for (int i = 0; i< board.length; i++){
                        for (int j = 0; j< board.length; j++){
                            if (this.nowPlayerColor().equals("White")){
                                if (board[i][j]!=null && board[i][j].getSymbol().equals("K") && board[i][j].getColor().equals("Black")){
                                ...
                                if (board[i][j]!=null && board[i][j].getSymbol().equals("K") && board[i][j].getColor().equals("White")){
                                    System.out.println(i + " / " + j );
                                    l = i;
                                    c = j;
                                    break;
                    ...
                    // check shah
                    if (new Queen(this.nowPlayerColor()).canMoveToPosition(this, endLine, endColumn, l, c) ||
                            new King(this.nowPlayerColor()).canMoveToPosition(this, endLine, endColumn, l, c) ||
                            new Horse(this.nowPlayerColor()).canMoveToPosition(this, endLine, endColumn, l, c) ||
                            new Rook(this.nowPlayerColor()).canMoveToPosition(this, endLine, endColumn, l, c) ||
                            new Bishop(this.nowPlayerColor()).canMoveToPosition(this, endLine, endColumn, l, c) ||
                            new Pawn(this.nowPlayerColor()).canMoveToPosition(this, endLine, endColumn, l, c))
                    {
